...
tape(`zlib gzip|gunzip a buffer`, (t) => {
  zlib.gzip(input, (err, buffer1) => {
    if (err) { return t.end(err) }
    zlib.gunzip(buffer1, (err, buffer2) => {
      if (err) { return t.end(err) }
...
tape('zlib.gunzip a truncated buffer', (t) => {
  zlib.gzip(input, (err, gzBuf) => {
    if (err) { return t.end(err) }

    const gzBufTruncated = gzBuf.slice(0, gzBuf.length / 2)
    zlib.gunzip(gzBufTruncated, (err, buffer) => {
      if (err) {
...
tape(`Handle error in truncated gunzip-maybe stream`, (t) => {
